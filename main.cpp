...
void fullscreen()
{
	...
	keybd_event(VK_MENU,0x40,KEYEVENTF_KEYUP,0);
}
...
int rate(int hour, int j)
{
	...
	hour_24=hour/12;
	hour_12=(hour - hour_24*12)/6;
	hour_1=(hour - hour_24*12 - hour_12*6)/1;
...
void displayCar()
{
	cout << "\t  |  House Number  Place\tPhone Number\tFloors   Rooms\t   Rate Per Month   Rate Per 6 Months   Rate Per 12 Months" << endl;
	cout << "\t  |  =======================================================================================================================" << endl;
...
void existingCust()
{
	...
	for(int i = 0; i < carCount(); i++)
	{
		if (strcmp(carSelect,rent[i].plate_num) == 0)
		{
			...
			log<<"\nHOUSE NUMBER: "<<rent[i].plate_num;
			log<<"\nPLACE: "<<rent[i].brand;
			log<<"\nPHONE NUMBER: "<<rent[i].model;
			log <<"\nMONTHS: "<< hour;
			log <<"\nPAYMENT: "<< rate(hour,j);
		}
	}
...
void newCustData ()
{
	...
	cout << "\n\t  |\tPrice for " << hour << " Months of rent : RM ";
	cout << rate(hour,j);
...
void newCarData()
{
	...
	cout << "\n\n";
	cout << "\n\t  |\t\t\t\t\t  Please enter the Home data below : " << endl;
	
	cout << "\n\t  |\t\t\t\t\t  House Number : ";
	cin >> ws;
	cin.getline(rent[newCar].plate_num,15);
	...
	cout << "\t  |\t\t\t\t\t  Phone Number : ";
	cin.getline(rent[newCar].model,30);
...
void carData()
{
	...
	while(!ifs.eof())
	{
		...
		ifs.getline(rent[carNum].model,30,' ');
...
void delCar()
{
	...
	cout << "\n\n";
	cout << "\t  | \t\tEnter the House number that are going to be deleted (CAPITAL LETTER W/OUT SPACE): "<<endl;
	cout<< "\t  |  \t\t\t\t\t\t\t";
	cin >> ws;
...
void tNc()
{
	art();
	cout<<endl;
	cout<<"\n\t  |\t\t\t1. HOUSES ARE LOCATED IN LOCALITY OF MANGALORE, KARNATAKA STATE";
	cout<<"\n\t  |\t\t\t2. MINIMUM RENT TIME IS 3 MONTHS";
	cout<<"\n\t  |\t\t\t3. RISK BY YOUR OWN";
	cout<<"\n\t  |\t\t\t4. INCASE OF ANY DAMAGES , EXTRA COSTING IS PAID BY THE CUSTOMER";
	cout<<"\n\t  |\t\t\t5. PREPARE ITEM AS BELOW";
	cout<<"\n\t  |\t\t\t     \3 COPY OF AADHAR CARD";
	cout<<"\n\t  |\t\t\t     \3 DEPOSIT (BASED ON HOME TYPE)";
